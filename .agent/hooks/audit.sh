...
cat <<'EOF'
{
  "systemMessage": "Project uses bd (beads) for issue tracking. Run `bd ready` to find available work. Follow the pipeline in .agent/rules/main.mdc: analyze → plan → implement → test → push."
}
EOF
